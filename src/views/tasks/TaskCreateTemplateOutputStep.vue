    ...
    <div class="step4-layout">
      <main class="step4-main">
        ...
        <footer class="step4-actions">
          <p class="step4-current-step">当前步骤 <strong>{{ currentStep + 1 }}</strong> / 5 <span v-if="validationMessage" class="validation-alert" role="alert"><AuditIcon name="anomaly" />{{ validationMessage }}</span><span v-else-if="summary.risks.length" class="step4-risk-inline"><AuditIcon name="anomaly" />{{ summary.risks[0] }}</span></p>
          <div class="step4-action-buttons"><button type="button" class="step4-button draft" @click="$emit('request-save')">保存草稿</button><button type="button" class="step4-button previous" @click="$emit('request-previous')">上一步</button><button type="button" class="step4-button submit" @click="handleSubmit">提交任务</button></div>
        </footer>
      </main>
...
@container(max-width:819px){.global-setting-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.step4-wizard{overflow-x:auto;padding:0 14px}.step4-wizard>i{flex:0 0 30px;margin:0 8px}.step4-actions{align-items:stretch;flex-direction:column;padding:12px}.step4-current-step{white-space:normal}.step4-action-buttons{display:grid;grid-template-columns:1fr 1fr}.step4-action-buttons>.draft{grid-column:1/-1}.step4-risk-inline{max-width:none}.step4-button{width:100%}}
@container(max-width:559px){.task-template-output-step{width:100%}.global-setting-grid{grid-template-columns:1fr}.step4-wizard-item b{display:none}.step4-title-row h2{font-size:20px}.step4-action-buttons{grid-template-columns:1fr}.step4-action-buttons>.draft{grid-column:auto}}
