...
export const taskDetailExecutionSnapshot = {
  ...
  stages: [
    ...
    { key: 'execute', label: '能力执行', detail: '09:45 开始', state: 'active' },
  ...
  capabilities: [
    {
      ...
      status: 'completed', statusLabel: '已完成', phaseDetail: '已生成', currentStep: '检索制度条款', dependency: '',
    ...
    },
    {
      ...
      status: 'completed', statusLabel: '已完成', phaseDetail: '已生成', currentStep: '生成变更时间轴', dependency: '',
      input: '制度版本 18 份', output: '制度变更清单.xlsx', actions: ['view-log', 'view-result']
    },
    {
      id: 'policy-compare', index: 3, name: '制度比对', description: '比对内外部制度差异',
      status: 'running', statusLabel: '执行中', phaseDetail: '匹配差异', currentStep: '匹配差异', dependency: '',
    ...
    },
    {
      ...
      status: 'running', statusLabel: '执行中', phaseDetail: '提取监管关注点', currentStep: '提取监管关注点', dependency: '',
    ...
    },
    {
      ...
      status: 'running', statusLabel: '执行中', phaseDetail: '生成审计条目', currentStep: '生成审计条目', dependency: '',
    ...
    },
    {
      ...
      status: 'completed', statusLabel: '已完成', phaseDetail: '已生成', currentStep: '生成汇总分析', dependency: '',
    ...
    },
    {
      ...
      status: 'running', statusLabel: '执行中', phaseDetail: '确认异常结果', currentStep: '确认异常结果', dependency: '',
    ...
    },
    {
      ...
      status: 'pending', statusLabel: '待执行', phaseDetail: '等待依赖', currentStep: '等待依赖', dependency: '费用审计确认完成',
      input: '—', output: '审计报告草稿.docx', actions: []
    },
    {
      id: 'report-review', index: 9, name: '报告审核', description: '审核报告格式与内容',
      status: 'pending', statusLabel: '待执行', phaseDetail: '等待依赖', currentStep: '等待依赖', dependency: '上传报告',
  ...
  outputFiles: [
    { id: 'OUT-1', category: '制度比对结果', name: '差异清单.xlsx', type: 'Excel', status: '已生成' },
    { id: 'OUT-2', category: '分析结果', name: '费用异常汇总.xlsx', type: 'Excel', status: '执行中' },
    { id: 'OUT-3', category: '审计规范', name: '审计规范与建议.xlsx', type: 'Excel', status: '执行中' },
    { id: 'OUT-4', category: '报告', name: '审计报告草稿.docx', type: 'Word', status: '待生成' }
  ],
  timelineFilters: [
    '全部', '创建任务', '上传文件', '解析文件', '确认结果', '排除结果', '生成报告', '导出文件', '提交复核', '执行异常'
  ].map((label) => ({ label, value: label === '全部' ? 'all' : label })),
  timelineEvents: [
    { id: 'TL-1', type: '创建任务', time: '2025-04-28 09:15:32', title: '创建任务', detail: '任务已创建并锁定执行参数', operator: '审计管理员' },
    { id: 'TL-2', type: '上传文件', time: '2025-04-28 09:18:04', title: '上传文件', detail: '上传制度、费用和共享文件', operator: '审计管理员' },
    { id: 'TL-3', type: '解析文件', time: '2025-04-28 09:22:18', title: '解析文件', detail: '制度文件解析完成', operator: '系统' },
    { id: 'TL-4', type: '执行异常', time: '2025-04-28 09:44:58', title: '共享资料章节缺失', detail: '已记录异常，等待后续补充资料', operator: '系统' }
  ]
};
...
export function getTaskDetailExecutionSnapshot(taskId, rows = taskRows) {
  ...
  snapshot.stages[2].detail = '09:45 开始';
  return snapshot;
}
