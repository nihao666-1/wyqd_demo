...
let taskDetailShell = null;

const activeTabLabel = computed(() => snapshot.tabs.find((tab) => tab.key === activeTab.value)?.label || '任务详情');

function updateTaskDetailScale() {
  if (!taskDetailShell) return;
  const viewportWidth = document.documentElement.clientWidth || window.innerWidth;
  const viewportHeight = document.documentElement.clientHeight || window.innerHeight;
  const scale = viewportWidth >= 1200 && viewportWidth < DESIGN_WIDTH ? viewportWidth / DESIGN_WIDTH : 1;
  const horizontalOffset = scale < 1 ? Math.max(0, (viewportWidth - DESIGN_WIDTH * scale) / 2 / scale) : 0;
  taskDetailShell.style.setProperty('--task-detail-scale', String(scale));
  taskDetailShell.style.setProperty('--task-detail-offset-x', `${horizontalOffset}px`);
  taskDetailShell.classList.toggle('task-detail-scaled', scale < 1);
  taskDetailShell.classList.toggle('task-detail-density-compact', viewportHeight <= 800 && viewportWidth >= 1200);
}
