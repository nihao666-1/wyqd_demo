...
const parseStatus = computed(() => props.file?.parseStatus || props.file?.status || '待处理');
const blockers = computed(() => normalizeList(props.file?.blockers));
const suggestions = computed(() => normalizeList(props.file?.suggestions));
const failureReason = computed(() => props.file?.failureReason || props.file?.errorMessage || (parseStatus.value === '解析异常' ? '文件内容无法被解析，请检查文件完整性或格式。' : ''));
...
const isBlocked = computed(() => typeof props.file?.blocked === 'boolean'
  ? props.file.blocked
  : blockers.value.length > 0 || parseStatus.value === '解析异常');
...
const timelineSteps = computed(() => {
  ...
  const parsingState = parseStatus.value === '解析异常' ? 'error' : parseStatus.value === '解析中' ? 'active' : parseStatus.value === '解析成功' ? 'complete' : 'pending';
  return [
...
function stateText(state) { return ({ complete: '已完成', active: '进行中', error: '解析异常', pending: '待处理' })[state] || state; }
