...
@media(max-width:1199px){.files-layout{grid-template-columns:minmax(0,1fr)}.file-tree-slot{position:fixed;z-index:29;top:58px;bottom:0;left:72px;width:260px;transform:translateX(calc(-100% - 72px));transition:transform .18s ease}.file-tree-slot.open{transform:translateX(0)}.precheck-grid{grid-template-columns:1fr 1fr}.blocking-column{grid-column:1/-1;border-top:1px solid var(--parse-line)}.center-stack{grid-template-rows:408px auto 64px}.parsing-flow-footer{position:sticky;bottom:0;z-index:4}}
...
@media(min-width:1200px) and (max-width:1499px){
  ...
  .workspace-grid{grid-template-columns:minmax(0,1fr) 358px;gap:10px;height:calc(var(--shell-viewport-height, 100vh) - 303px);min-height:620px;max-height:none}
  ...
  .center-stack{display:grid;grid-template-rows:minmax(417px,1fr) minmax(239px,.58fr) 60px}.precheck-grid{grid-template-columns:minmax(285px,1.25fr) minmax(150px,.65fr) minmax(260px,1.1fr)}.blocking-column{grid-column:auto;border-top:0}.parsing-flow-footer{position:static;align-items:center;flex-direction:row;padding:8px 10px}.parsing-flow-footer>p{white-space:nowrap}.parsing-footer-actions{display:flex}.flow-btn,.flow-btn.outline,.flow-btn.primary{width:auto}.flow-btn.outline{min-width:126px}.flow-btn.primary{min-width:220px}
