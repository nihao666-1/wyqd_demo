...
test('费用审计分析总览页是独立主页面，不再包装异常监控页', () => {
  ...
  for (const text of ['费用总额', '预算使用率', '异常金额', '高风险异常数']) assert.match(overview, new RegExp(text));
...
test('费用审计分析和异常监控使用分离路由与面包屑', () => {
  ...
  assert.match(router, /\{ path: '\/expense\/audit\/overview', component: ExpenseAuditOverview, meta: \{ title: '费用审计分析' \} \}/);
  assert.match(router, /\{ path: '\/expense\/anomaly\/dashboard', component: ExpenseAnomalyDashboard, meta: \{ title: '费用异常监控' \} \}/);
  assert.match(layout, /v-else-if="isExpenseAuditOverview" class="task-breadcrumb"><span>审计工作台<\/span><i>\/<\/i><strong>费用审计分析<\/strong><\/div>/);
  assert.match(layout, /v-else-if="isExpenseAnomalyMonitor" class="task-breadcrumb"><span>审计工作台<\/span><i>\/<\/i><span>费用审计分析<\/span><i>\/<\/i><strong>费用异常监控<\/strong><\/div>/);
  assert.match(layout, /\{ label: '费用审计分析', path: '\/expense\/audit\/overview' \}/);
  assert.match(layout, /\{ label: '费用异常监控', path: '\/expense\/anomaly\/dashboard' \}/);
});
