...
test('草稿右栏包含资料输出阻断和当前任务日志，不再跳转系统管理', () => {
  const content = source('TaskDetailDraftRail.vue');
  for (const label of ['下一步提示', '待补充资料', '所需输出', '阻断项', '操作留痕', '10 条/页']) assert.match(content, new RegExp(label));
  assert.doesNotMatch(content, /全部日志|\/system/);
