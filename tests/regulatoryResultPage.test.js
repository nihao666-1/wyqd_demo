...
test('监管结果路由拥有专用 shell、面包屑和可构建工作台文件', () => {
  ...
  assert.match(layout, /v-else-if="isRegulatoryResultRoute" class="task-breadcrumb"><span>审计知识库<\/span><i>\/<\/i><strong>监管案例与舆情分析<\/strong><\/div>/);
  assert.match(layout, /isKnowledgeSection[^}]*item\.path === ['"]\/audit-standard\/policy['"]/s);
