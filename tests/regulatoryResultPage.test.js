...
test('监管结果页图表区在桌面端一行展示四张图表', () => {
  const source = read(resultPageUrl);

  assert.match(
    source,
    /\.chart-grid\s*\{[^}]*grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*1fr\)\)/s
  );
});

test('高频处罚原因横轴的五个刻度保持同一行', () => {
  const source = read(resultPageUrl);
  const declaredColumns = [...source.matchAll(/\.reason-axis\s*\{([^}]*)\}/g)]
    .map(([, body]) => body.match(/grid-template-columns:\s*([^;]+);/)?.[1])
    .filter(Boolean);

  assert.equal(declaredColumns.at(-1).replace(/\s+/g, ''), 'repeat(5,1fr)');
});
...
test('监管结果路由拥有专用 shell、面包屑和可构建工作台文件', () => {
  ...
  assert.match(css, /\.regulatory-result-shell \.sidebar[\s\S]*width:\s*var\(--shell-sidebar-width\)[^}]*flex:\s*0 0 var\(--shell-sidebar-width\)/s);
  assert.match(css, /\.regulatory-result-shell \.main[\s\S]*min-height:\s*var\(--shell-viewport-height\)/s);
  assert.doesNotMatch(css, /--regulatory-result-scale/);
  assert.doesNotMatch(css, /transform:\s*scale\(/);
});

test('监管案例新建入口进入创建审计任务并预选监管案例舆情分析', () => {
  const result = read(resultPageUrl);
  const workbench = read(workbenchUrl);
  const router = read(routerUrl);

  assert.match(result, /function createRegulatoryAnalysis\(\)[\s\S]*router\.push\(\{\s*path:\s*['"]\/tasks\/create['"],\s*query:\s*\{\s*capability:\s*['"]regulatory['"]\s*\}\s*\}\)/);
  assert.match(workbench, /to:\s*\{\s*path:\s*['"]\/tasks\/create['"],\s*query:\s*\{\s*capability:\s*['"]regulatory['"]\s*\}\s*\}/);
  assert.match(workbench, /to:\s*\{\s*path:\s*['"]\/tasks\/create['"],\s*query:\s*\{\s*capability:\s*['"]supervision['"]\s*\}\s*\}/);
  assert.match(router, /path:\s*['"]\/regulatory\/new['"][\s\S]*redirect:\s*\{\s*path:\s*['"]\/tasks\/create['"],\s*query:\s*\{\s*capability:\s*['"]regulatory['"]\s*\}\s*\}/);
});
