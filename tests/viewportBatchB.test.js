...
test('batch B workspaces opt into fill while forms remain content driven', () => {
  ...
  for (const path of deletedShortStepPages) {
    assert.equal(existsSync(new URL(path, import.meta.url)), false, `${path} should be removed after route consolidation`);
  }
});
...
test('batch B result pages keep overflow inside their main and detail regions', () => {
  ...
  const library = read('../src/views/audit-standard/AuditStandardLibrary.vue');
  const workbench = read('../src/views/regulatory/RegulatoryWorkbench.vue');
  assert.match(library, /panel=diff|versionDiff|merge-hint|版本差异/s);
