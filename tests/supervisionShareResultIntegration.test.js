...
test('结果路由仅启用专用壳并显示目标面包屑', () => {
  ...
  assert.match(layout, /v-else-if="isSupervisionShareResultRoute" class="task-breadcrumb"><span>审计知识库<\/span><i>\/<\/i><span>上海分公司Q1常规审计任务<\/span><i>\/<\/i><strong>监督共享信息分析<\/strong><\/div>/);
...
test('结果壳选中审计知识库并提供基线尺寸与颜色', () => {
  assert.match(layout, /['"]manual-active['"]:\s*\([^}]*isKnowledgeSection[^}]*item\.path === ['"]\/audit-standard\/policy['"]/s);
...
test('结果页使用自适应网格、紧凑详情抽屉和卸载清理', () => {
  ...
  assert.match(resultPage, /\.source-detail-card\s*\{[^}]*width:\s*100%[^}]*height:\s*auto[^}]*overflow:\s*visible/s);
  assert.match(resultPage, /@media \(max-width: 1499px\)[\s\S]*\.supervision-board\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\)/s);
...
test('结果页统一收紧字号、图表、操作列和详情日志', () => {
  ...
  assert.match(resultPage, /\.bar-chart,\s*\.donut-layout\s*\{[^}]*min-height:\s*118px/s);
