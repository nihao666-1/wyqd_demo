...
test('结果路由仅启用专用壳并显示目标面包屑与通知角标', () => {
  ...
  assert.match(layout, /v-if="!isParsingPhase && !isTaskDetailRoute && !isAuditStandardResult && !isSupervisionShareResultRoute[^"]*" class="global-data-mode"/);
...
test('结果壳选中专项审计分析并提供基线尺寸与颜色', () => {
  ...
  assert.match(css, /\.supervision-result-shell \.sidebar\s*\{[^}]*width:\s*206px[^}]*flex:\s*0 0 206px[^}]*background:\s*#fff/s);
  assert.match(css, /\.supervision-result-shell \.brand\s*\{[^}]*min-height:\s*57px[^}]*background:\s*#c9000b/s);
  assert.match(css, /\.supervision-result-shell \.topbar\s*\{[^}]*min-height:\s*58px[^}]*height:\s*58px/s);
  assert.match(css, /\.supervision-result-shell \.main-nav a\.manual-active\s*\{[^}]*color:\s*#c9000b/s);
  assert.match(css, /\.supervision-result-shell \.main-nav a\.manual-active::before\s*\{[^}]*background:\s*#c9000b/s);
});
...
test('结果页实现单一缩放生命周期、紧凑右栏和卸载清理', () => {
  assert.match(resultPage, /const REFERENCE_WIDTH = 1586/);
  assert.match(resultPage, /const COMPACT_DETAIL_BREAKPOINT = 1280/);
  assert.match(resultPage, /window\.innerWidth < COMPACT_DETAIL_BREAKPOINT/);
  assert.match(resultPage, /--supervision-result-scale/);
  assert.match(resultPage, /--supervision-result-compact-scale/);
  assert.match(resultPage, /supervision-result-scaled/);
  assert.match(resultPage, /supervision-result-compact/);
  assert.match(resultPage, /supervision-result-compact-height/);
  assert.match(resultPage, /window\.addEventListener\('resize', applyViewportState\)/);
  assert.match(resultPage, /window\.removeEventListener\('resize', applyViewportState\)/);
  ...
  assert.match(resultPage, /(?:document\.documentElement|root)\.style\.removeProperty\('--supervision-result-scale'\)/);
  assert.match(resultPage, /(?:document\.documentElement|root)\.style\.removeProperty\('--supervision-result-compact-scale'\)/);
  assert.match(resultPage, /isCompactDetail/);
  assert.match(resultPage, /class="detail-overlay"/);
  assert.match(resultPage, /Escape/);
  assert.match(resultPage, /\.supervision-page\s*\{[^}]*grid-template-columns:\s*1034px 303px[^}]*width:\s*1351px/s);
  assert.match(resultPage, /\.source-detail-card\s*\{[^}]*top:\s*57px[^}]*width:\s*303px[^}]*height:\s*741px/s);
  assert.match(css, /html\.supervision-result-compact \.supervision-page\s*\{[^}]*width:\s*1351px/s);
});
